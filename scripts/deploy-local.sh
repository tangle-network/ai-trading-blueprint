...
DEFAULT_AI_PROVIDER_FOR_ENV="${VITE_DEFAULT_AI_PROVIDER:-}"
DEFAULT_AI_KEY_FOR_ENV="${VITE_DEFAULT_AI_API_KEY:-}"
if [[ -z "$DEFAULT_AI_PROVIDER_FOR_ENV" ]]; then
  if [[ -n "${GOOGLE_AI_KEY:-}" || -n "${GEMINI_API_KEY:-}" ]]; then
    DEFAULT_AI_PROVIDER_FOR_ENV="gemini"
  elif [[ -n "${ZAI_API_KEY:-}" ]]; then
    DEFAULT_AI_PROVIDER_FOR_ENV="zai"
  fi
fi
if [[ -z "$DEFAULT_AI_KEY_FOR_ENV" ]]; then
  case "$DEFAULT_AI_PROVIDER_FOR_ENV" in
    gemini) DEFAULT_AI_KEY_FOR_ENV="${GOOGLE_AI_KEY:-${GEMINI_API_KEY:-}}" ;;
    zai) DEFAULT_AI_KEY_FOR_ENV="${ZAI_API_KEY:-}" ;;
    tangle-router) DEFAULT_AI_KEY_FOR_ENV="${TANGLE_API_KEY:-}" ;;
  esac
fi

existing_arena_env() {
  local key="$1"
  if [[ -f arena/.env.local ]]; then
    sed -n "s/^${key}=//p" arena/.env.local | tail -n 1
  fi
}

HYPEREVM_MAINNET_ENABLED_FOR_ENV="${VITE_HYPEREVM_MAINNET_ENABLED:-$(existing_arena_env VITE_HYPEREVM_MAINNET_ENABLED)}"
HYPEREVM_MAINNET_CHAIN_ID_FOR_ENV="${VITE_HYPEREVM_MAINNET_CHAIN_ID:-$(existing_arena_env VITE_HYPEREVM_MAINNET_CHAIN_ID)}"
HYPEREVM_MAINNET_RPC_URL_FOR_ENV="${VITE_HYPEREVM_MAINNET_RPC_URL:-$(existing_arena_env VITE_HYPEREVM_MAINNET_RPC_URL)}"
HYPEREVM_MAINNET_USDC_FOR_ENV="${VITE_HYPEREVM_MAINNET_USDC_ASSET_TOKEN:-$(existing_arena_env VITE_HYPEREVM_MAINNET_USDC_ASSET_TOKEN)}"
HYPEREVM_MAINNET_FACTORY_FOR_ENV="${VITE_HYPEREVM_MAINNET_VAULT_FACTORY_ADDRESS:-$(existing_arena_env VITE_HYPEREVM_MAINNET_VAULT_FACTORY_ADDRESS)}"
HYPEREVM_MAINNET_VAULT_FOR_ENV="${VITE_HYPEREVM_MAINNET_VAULT_ADDRESS:-$(existing_arena_env VITE_HYPEREVM_MAINNET_VAULT_ADDRESS)}"
HYPEREVM_MAINNET_PAPER_TRADE_FOR_ENV="${VITE_HYPEREVM_MAINNET_PAPER_TRADE:-$(existing_arena_env VITE_HYPEREVM_MAINNET_PAPER_TRADE)}"

DEPLOYMENT_FINGERPRINT_FOR_ENV="${VITE_DEPLOYMENT_FINGERPRINT:-$(existing_arena_env VITE_DEPLOYMENT_FINGERPRINT)}"

cat > arena/.env.local <<EOF
VITE_USE_LOCAL_CHAIN=true
VITE_DEPLOYMENT_FINGERPRINT=$DEPLOYMENT_FINGERPRINT_FOR_ENV
...
VITE_HYPEREVM_MAINNET_ENABLED=${HYPEREVM_MAINNET_ENABLED_FOR_ENV:-false}
VITE_HYPEREVM_MAINNET_CHAIN_ID=${HYPEREVM_MAINNET_CHAIN_ID_FOR_ENV:-}
VITE_HYPEREVM_MAINNET_RPC_URL=$HYPEREVM_MAINNET_RPC_URL_FOR_ENV
VITE_HYPEREVM_MAINNET_USDC_ASSET_TOKEN=$HYPEREVM_MAINNET_USDC_FOR_ENV
VITE_HYPEREVM_MAINNET_VAULT_FACTORY_ADDRESS=$HYPEREVM_MAINNET_FACTORY_FOR_ENV
VITE_HYPEREVM_MAINNET_VAULT_ADDRESS=$HYPEREVM_MAINNET_VAULT_FOR_ENV
VITE_HYPEREVM_MAINNET_PAPER_TRADE=${HYPEREVM_MAINNET_PAPER_TRADE_FOR_ENV:-false}
...
VITE_DEFAULT_AI_PROVIDER=$DEFAULT_AI_PROVIDER_FOR_ENV
VITE_DEFAULT_AI_API_KEY=$DEFAULT_AI_KEY_FOR_ENV
