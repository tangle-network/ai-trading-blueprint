...
arena_env_value() {
  local key="$1"
  awk -F= -v key="$key" '$1 == key { sub($1"=", ""); print; exit }' "$ROOT_DIR/arena/.env.local"
}
...
LOCAL_USDC_ADDRESS="$(arena_env_value VITE_USDC_ADDRESS)"
LOCAL_WETH_ADDRESS="$(arena_env_value VITE_WETH_ADDRESS)"
LOCAL_ASSET_TOKEN_ADDRESS="${ASSET_TOKEN_ADDRESS:-$LOCAL_USDC_ADDRESS}"
if [[ -z "$LOCAL_ASSET_TOKEN_ADDRESS" || -z "$LOCAL_USDC_ADDRESS" || -z "$LOCAL_WETH_ADDRESS" ]]; then
  echo "ERROR: deploy-local did not write local asset addresses to arena/.env.local" >&2
  echo "  ASSET_TOKEN_ADDRESS=$LOCAL_ASSET_TOKEN_ADDRESS" >&2
  echo "  USDC_ADDRESS=$LOCAL_USDC_ADDRESS" >&2
  echo "  WETH_ADDRESS=$LOCAL_WETH_ADDRESS" >&2
  exit 1
fi
EXECUTION_VAULT_MANIFEST="${EXECUTION_VAULT_MANIFEST:-$ROOT_DIR/deployments/${EXECUTION_CHAIN_ID:-$CHAIN_ID}/hyperliquid-vault.json}"
EXECUTION_TRADE_VALIDATOR_FROM_MANIFEST=""
if [[ -f "$EXECUTION_VAULT_MANIFEST" ]]; then
  EXECUTION_TRADE_VALIDATOR_FROM_MANIFEST="$(ruby -rjson -e 'puts JSON.parse(File.read(ARGV[0])).fetch("tradeValidator", "")' "$EXECUTION_VAULT_MANIFEST")"
fi
...
VALIDATOR_VERIFYING_CONTRACT="${EXECUTION_TRADE_VALIDATOR_ADDRESS:-${EXECUTION_TRADE_VALIDATOR:-${EXECUTION_TRADE_VALIDATOR_FROM_MANIFEST:-$TRADE_VALIDATOR_ADDRESS}}}"
...
ASSET_TOKEN_ADDRESS="$LOCAL_ASSET_TOKEN_ADDRESS" \
USDC_ADDRESS="$LOCAL_USDC_ADDRESS" \
WETH_ADDRESS="$LOCAL_WETH_ADDRESS" \
OPERATOR_ADDRESS="0x70997970C51812dc3A010C7d01b50e0d17dc79C8" \
PRIVATE_KEY="0x59c6995e998f97a5a0044966f0945389dc9e86dae88c7a8412f4603b6b78690d" \
HYPERLIQUID_API_WALLET_PRIVATE_KEY="${HYPERLIQUID_API_WALLET_PRIVATE_KEY:-0x59c6995e998f97a5a0044966f0945389dc9e86dae88c7a8412f4603b6b78690d}" \
TANGLE_CONTRACT="0xCf7Ed3AccA5a467e9e704C703E8D87F634fB0Fc9" \
STAKING_CONTRACT="0xe7f1725E7734CE288F8367e1Bb143E90bb3F0512" \
STATUS_REGISTRY_CONTRACT="0x99bbA657f2BbC93c02D617f8bA121cB8Fc104Acf" \
...
HYPERLIQUID_API_WALLET_PRIVATE_KEY="${HYPERLIQUID_API_WALLET_PRIVATE_KEY:-0x5de4111afa1a4b94908f83103eb1f1706367c2e68ca870fc3fb9a804cdab365a}" \
TANGLE_CONTRACT="0xCf7Ed3AccA5a467e9e704C703E8D87F634fB0Fc9" \
STAKING_CONTRACT="0xe7f1725E7734CE288F8367e1Bb143E90bb3F0512" \
STATUS_REGISTRY_CONTRACT="0x99bbA657f2BbC93c02D617f8bA121cB8Fc104Acf" \
SESSION_AUTH_SECRET="${SESSION_AUTH_SECRET:-dev-secret-key-do-not-use-in-production}" \
