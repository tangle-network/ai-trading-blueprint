...
export function ArenaAppShell() {
  ...
  return (
    <div className="bp-tone-arena flex h-[100dvh] overflow-hidden bg-arena-elements-background-depth-1 text-arena-elements-textPrimary bg-mesh bg-noise">
      ...
      <aside className="relative z-10 hidden w-64 shrink-0 flex-col border-r border-arena-elements-dividerColor/70 bg-arena-elements-background-depth-2/86 shadow-[12px_0_38px_rgba(0,0,0,0.18)] backdrop-blur-xl lg:flex">
        <div className="flex h-14 shrink-0 items-center border-b border-arena-elements-dividerColor/70 px-4">
          ...
          {primaryNavItems.map((item) => {
            ...
            return (
              <NavLink
                ...
                to={item.href}
                className={cn(
                  'flex h-10 items-center gap-2.5 rounded-lg px-3 text-sm font-display font-medium transition-colors',
                  active
        ...
        <div className="min-h-0 flex-1 border-t border-arena-elements-dividerColor/60 px-2 py-3">
          ...
          <div className="space-y-1 overflow-y-auto pr-1">
            {activeAgents.length === 0 ? (
              <div className="rounded-lg border border-dashed border-arena-elements-dividerColor/70 px-3 py-4 text-xs text-arena-elements-textTertiary">
                {isConnected ? 'No active agents yet.' : 'Connect to load operator agents.'}
              </div>
            ) : activeAgents.map((bot) => {
              ...
              return (
                <Link
                  ...
                  className={cn(
                    'group flex min-h-[56px] items-center gap-2 rounded-lg border px-2 py-2 transition-colors',
                    selected
