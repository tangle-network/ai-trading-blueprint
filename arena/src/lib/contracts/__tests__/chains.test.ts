...
describe('HyperEVM chain configuration', () => {
  ...
  it('does not configure HyperEVM mainnet when required addresses are zero', async () => {
    ...
    vi.stubEnv('VITE_HYPEREVM_MAINNET_VAULT_ADDRESS', VAULT);
  ...
  it('configures HyperEVM only when RPC, asset token, factory, and vault are non-zero', async () => {
    vi.stubEnv('VITE_HYPEREVM_TESTNET_ENABLED', 'true');
    vi.stubEnv('VITE_HYPEREVM_TESTNET_CHAIN_ID', '998');
    vi.stubEnv(
      'VITE_HYPEREVM_TESTNET_RPC_URL',
