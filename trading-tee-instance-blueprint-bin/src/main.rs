//! Binary runner for the AI trading TEE instance blueprint.
//!
//! TEE-backed variant of the instance binary: reads `TEE_BACKEND` env var to
//! select the backend, then routes provision/deprovision through it.
//! Supports Phala, AWS Nitro, GCP Confidential Space, Azure SKR, and
...
async fn main() -> Result<(), blueprint_sdk::Error> {
    ...
    tracing::info!("TEE backend initialized (type: {backend_type})");
    ...
    if env.registration_mode() {
        ...
        tracing::info!(
            "Trading TEE instance registration payload written to {}",
            path.display()
        );
        return Ok(());
    }
    ...
    tracing::info!("Starting trading TEE instance blueprint for service {service_id}");
    ...
    tracing::info!("Skipping on-chain workflow bootstrap for trading TEE instance blueprint");
    ...
    // ── 7. Spawn operator API server ─────────────────────────────────────
    {
        ...
        tracing::info!("TEE instance operator API listening on 0.0.0.0:{port}");
        tokio::spawn(async move {
    ...
    // ── 8. Set up Tangle producer/consumer + cron workflow tick ───────────
    ...
    let result = BlueprintRunner::builder(tangle_config, env)
        ...
        .with_shutdown_handler(async {
            ...
            match trading_instance_blueprint_lib::require_instance_bot() {
                ...
                Err(e) => tracing::info!("Shutdown: no instance to deprovision ({e})"),
            }
        })
        .run()
        .await;
