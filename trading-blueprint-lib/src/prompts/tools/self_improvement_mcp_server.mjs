...
function makeCodingPrompt(task, variant, round) {
  return [
    'You are auto-dev inside a trading bot sandbox.',
    ...
    '- Do not touch live keys, secrets, validator credentials, or vault funds.',
    '- Make the smallest coherent code change that satisfies the spec.',
    '- Add/update tests where the change needs proof.',
    ...
    'Leave the intended patch in the worktree when finished.',
  ].join('\n');
}
