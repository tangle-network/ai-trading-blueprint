...
5. If the setup is actionable, build a `supply`, `withdraw`, `borrow`, or `repay` intent with chain-specific addresses from `aave-reserve-status.js`, `amount_format: "base_units"`, and `target_protocol: "aave_v3"`, or a `supply`/`withdraw` intent with `target_protocol: "morpho_vault"` and `metadata.vault_address` from the allowlist, validate it, then execute it if approved using `const validation = await api.validate(intent); if ((validation.data||validation).approved) await api.execute(intent, validation);`
